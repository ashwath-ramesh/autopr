...
func (r *Runner) runImplement(ctx context.Context, jobID string, issue db.Issue, projectCfg *config.ProjectConfig, workDir string) error {
	...
	// Update commit SHA if the LLM committed changes.
	if resp.CommitSHA != "" {
		_ = r.store.UpdateJobField(ctx, jobID, "commit_sha", resp.CommitSHA)
	}

	slog.Info("implement step completed", "job", jobID, "commit", resp.CommitSHA)
	return nil
}
